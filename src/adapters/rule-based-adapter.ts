import { PromptPackage, ReasoningResult, ReasoningInput, ReasoningStrategy, ReasoningProvider, ExperienceArchitecture, DesignRationale, OpenQuestion } from '../reasoning.js';
...
export class RuleBasedStrategy implements ReasoningStrategy {
  readonly name = 'rule-based';

  buildPromptPackage(input: ReasoningInput): PromptPackage {
    return {
      systemInstructions: 'Rule-based reasoning - no external prompt needed',
      userPrompt: input.requirements,
      responseFormat: 'json',
...
export class RuleBasedAdapter implements ReasoningProvider, ReasoningAdapter {
  readonly name = 'rule-based';
